...
class Game:
    ...
    def simulate_possession(self, remaining_secs, shot_clock):
        ...
        while shot_clock > 0 and rem > 0:
            ...
            turnover_prob = min(safe_attr(ball_handler, "tpg", 0.0) / gp_bh * 0.06 + 0.02, 0.05)
            ...
            shooting_volume = min(bh_FGM / max(gp_bh, 1) / 20.0, 1.0)
            ...
            if action == "pass":
                ...
                if not teammates:
                    ...
                    new_handler = random.choice(teammates)
            ...
            # --- DRIBBLE ACTION ---
                ...
                for defender in defenders_on_court:
                    ...
                    if random.random() < foul_factor * 0.4:
            ...
            # --- SHOT ACTION ---
                ...
                three_point_rate = shooter_FG3A / max(shooter_FGA, 1) if shooter_FGA else 0.1
                ...
                if made:
                    ...
                    if random.random() < team_off_reb_rate and players_on_court:
                        ...
                        for p in players_on_court:
                            ...
                            w = (rpg * 1.5) if rpg > 6.5 else max(rpg, 0.01)
                        ...
                        for p in defenders_on_court:
                            rpg = safe_attr(p, "rpg", 0.0)
                            w = (rpg * 1.2) if rpg > 6.5 else max(rpg, 0.01)
                            d_weights.append(w)
...
def build_cumulative_data(team, year, json_folder="seasons_json"):
    ...
    for player in team:
        ...
        for season in previous_seasons:
            ...
            # Normalize all player names in the JSON file
            ...
            if player_norm in season_data:
                ...
                # Fuzzy match fallback (for small typos or variations)
        ...
        # Skip player entirely if not found in any season
        if not player_found:
            print(f"⚠️  No data found for player '{player}' in any season file.")
            continue
