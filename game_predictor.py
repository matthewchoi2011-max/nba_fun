...
class Game:
    ...
    def perform_substitution(self, team, actions, players_on_court, original_starters, total_game_seconds=48*60):
        MIN_STINT = 60 * 6
        STARTER_CAP = total_game_seconds * 0.75
        STARTER_MIN_RATIO = 0.8
        STARTER_STAY_CHANCE = 0.95

        # Compute minimum seconds for each original starter
        min_seconds_map = {p.name: STARTER_CAP * STARTER_MIN_RATIO for p in original_starters}

        # --- Identify starters eligible to sub out ---
        eligible_starters = [
            s for s in team.starters
            if (s.total_seconds >= min_seconds_map.get(s.name, 0) or s.box.foul >= 6)
            and s.stint_seconds >= MIN_STINT
        ]

        if not eligible_starters:
            return actions  # no sub needed

        # Pick starter to sub out
        over_cap_starters = [s for s in eligible_starters if s.total_seconds >= STARTER_CAP or s.box.foul >= 6]
        if over_cap_starters:
            sub_out = max(over_cap_starters, key=lambda p: p.total_seconds)
        else:
            contenders = [s for s in eligible_starters if random.random() > STARTER_STAY_CHANCE]
            sub_out = min(contenders, key=lambda p: p.box.plus_minus) if contenders else min(eligible_starters, key=lambda p: p.box.plus_minus)

        # --- Select bench candidates ---
        bench_candidates = [b for b in team.bench if b.total_seconds < STARTER_CAP and b.box.foul < 6]
    ...
    def simulate_possession(self, remaining_secs, shot_clock):
        ...
        while shot_clock > 0 and rem > 0:
            ...
            turnover_prob = min(ball_handler.tpg / max(ball_handler.gp,1) * 0.06 + 0.02, 0.05)
            ...
            # --- DRIBBLE ACTION ---
                ...
                for defender in defenders_on_court:
                    ...
                    if random.random() < foul_factor * 0.7:  # 2% chance per defender per action
                        defender.box.foul += 1
        ...
        MIN_POSSESSIONS_BETWEEN_SUBS = 7
